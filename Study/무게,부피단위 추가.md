...
func devideArrToStr (_ input: [String]) -> (String, String, String) {
    ...
    if input.count == 2 {
        ...
        from = findUnit(input)
        let frontString = input[0]
        var converToAll: [String] = []
        num += frontString.prefix(frontString.count - from.count)
        switch from {
        case UnitsName.Length.cm.rawValue :
            for unitName in UnitsTypeAndValue.length.keys {
                if unitName != UnitsName.Length.cm.rawValue {
                    converToAll.append(unitName)
                }
            }
            to = UnitsName.Length.m.rawValue
...
struct Length {
    enum Length: String { case cm, m, inch, yard }
    static let name = [Length.cm.rawValue, Length.m.rawValue, Length.inch.rawValue, Length.yard.rawValue]
    static let nameAndValue = [Length.cm.rawValue: 1, Length.m.rawValue: 100, Length.inch.rawValue: 2.54, Length.yard.rawValue: 91.44]
}

struct Weight {
    enum Weight: String { case g, kg, oz, lb }
    static let name = [Weight.g.rawValue, Weight.kg.rawValue, Weight.oz.rawValue, Weight.lb.rawValue]
    static let nameAndValue = [Weight.g.rawValue: 1, Weight.kg.rawValue: 1000, Weight.oz.rawValue: 28.3495, Weight.lb.rawValue: 453.592]
}

struct Volume {
    enum Volume: String { case l, gal, pt, qt }
    static let name = [Volume.l.rawValue, Volume.gal.rawValue, Volume.pt.rawValue, Volume.qt.rawValue]
    static let nameAndValue = [Volume.l.rawValue: 1, Volume.gal.rawValue: 0.473176, Volume.pt.rawValue: 0.946353, Volume.qt.rawValue: 3.78541]
}
```
