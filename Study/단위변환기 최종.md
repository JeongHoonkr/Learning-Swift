...
3. 문제해결과정
   ...
   * 추가된 구현사항 2가지 구현을 위해 추가적으로 체크 및 연산기능 구현

     > 함수가 쭉 나열되어 있어 읽기 힘든 코드가 되는것 같아서 공통되는 기능끼리 묶기로 했다.
     >
     > 그래서 분리클래스, 체크클래스, 연산클래스로 분할하였다.

   * 클래스간의 상속관계를 정하는데 어려움이 있었다.

     > 프로그램 구현 흐름에 따라 아래와 같이 상속관계를 정리하였다.
     >
